...
# from farm_ng_core_pybind import Pose
import time
...
async def go_to_goal(service_config: EventServiceConfig, track: Track) -> None:
    """Go to a goal.

    Args:
        service_config (EventServiceConfig): The track_follower service config.
        goal (TrackFollowerState): The goal to go to.
    """
    goal = track.waypoints[-7] # last waypoint
    tic = time.time()
    await EventClient(service_config).request_reply("/go_to_goal", goal)
    toc = time.time()
    print(f"Go to goal took {toc - tic} seconds")

async def cancel(service_config: EventServiceConfig) -> None:
    """Cancel the track_follower.

    Args:
        service_config (EventServiceConfig): The track_follower service config.
    """
    print("Sending request to stop the track_follower...")
...
async def main(service_config_path: Path, track_path: Path) -> None:
    ...
    # Go to the goal
    await go_to_goal(service_config, track)

    # Cancel the track_follower
    # await asyncio.sleep(3.0)
    # await cancel(service_config)
