...
def update_tool_state(tool_state: dict[int, bool], track_size: int, state: bool) -> None:
    """Update the tool state at a given index."""
    for i in range(len(tool_state), track_size):
        tool_state[i] = state
    return tool_state
...
def plot_track(waypoints: list[list[float]], tool_state: dict) -> None:
    ...
    for i, state in tool_state.items():
        if state:
            plt.plot(x[i], y[i], marker="o", markersize=5, color='green')
        else:
...
async def build_track(
    reverse: bool, event_manager: EventClientSubscriptionManager | None = None, save_track: Path | None = None
) -> Track:
    ...
    start: Pose3F64 = await create_start_pose(event_manager.clients["filter"])
    ...
    tool_state = update_tool_state(tool_state, track_builder.track_size, True)
    ...
    tool_state = update_tool_state(tool_state, track_builder.track_size, False)
    ...
    if save_track is not None:
        ...
        # Convert the tool_state dictionary to JSON
        tool_state_json = json.dumps(tool_state)

        # Save the JSON to a file
        with open("tool_state.json", "w") as file:
            tool_state_file = save_track.parent / "tool_state.json"
            with open(tool_state_file, "w") as file:
                file.write(tool_state_json)
...
async def run(args) -> None:
    ...
    tasks: list[asyncio.Task] = [asyncio.create_task(build_track(reverse, event_manager, save_track))]
